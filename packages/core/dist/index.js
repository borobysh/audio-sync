...
__export(index_exports, {
  ...
  AbstractMediaSession: () => AbstractMediaSession,
...
function createLogger(moduleName, instanceId) {
  const prefix = instanceId ? `[${moduleName}:${instanceId.slice(0, 4)}]` : `[${moduleName}]`;
  return (...args) => {
    if (DEBUG) {
...
var Driver = class extends AbstractDriver {
  audio;
...
var SyncCoordinator = class {
  ...
  _lastLeaderMessageTimestamp = 0;
  _leaderCheckCallback = null;
  _leaderCheckTimeoutId = null;
   ...
   */
  checkForActiveLeader(callback, timeout = 200) {
    if (this._isLeader) {
      callback(true);
      return;
    }
    const timeSinceLastLeaderMessage = Date.now() - this._lastLeaderMessageTimestamp;
    if (timeSinceLastLeaderMessage < 3e3) {
      callback(true);
      return;
    }
    ...
    this._leaderCheckTimeoutId = setTimeout(() => {
      if (this._leaderCheckCallback) {
        this._leaderCheckCallback(false);
      ...
      try {
        ...
        if (payload.isLeader && ["PLAY", "PAUSE", "STATE_UPDATE"].includes(type)) {
          ...
          if (this._leaderCheckCallback) {
            ...
            this._leaderCheckCallback(true);
            this._leaderCheckCallback = null;
          }
        }
  ...
  _handlePlay(payload, latency) {
    ...
    if (this._config.singlePlayback) {
      ...
      if (isSourceChanging && !this._config.syncTrackChange) {
        ...
        this._log("\u{1F3B5} Playing new track:", payload.currentSrc);
  ...
  _handleStateUpdate(payload, timestamp, latency) {
    ...
    if (this._config.singlePlayback) {
      ...
      if (payload.isPlaying && !this._engine.state.isPlaying) {
        ...
        this._log("\u{1F504} Remote changed track:", payload.currentSrc);
        this._driver.play(payload.currentSrc || void 0);
      }
    }
  }
